...
export async function fetchCalls() {
  ...
  const { data, error } = await supabase
    .from('calls')
    .select('id, created_at, customer_phone, customer_name, service_id, preferred_date, booking_id, summary, transcript_full, status, recording_url, conversation_state, bookings!booking_id(starts_at, services(name))')
